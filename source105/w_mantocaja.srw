HA$PBExportHeader$w_mantocaja.srw
forward
global type w_mantocaja from w_manto
end type
end forward

global type w_mantocaja from w_manto
end type
global w_mantocaja w_mantocaja
...
//***************************************************************************************************************************************************************************************
SELECT Max(caja.codcaja)   INTO :ls_codigo  FROM caja WHERE caja.idempresa = :gn_config.is_config.empresa Using SQLCA  ;
...
on w_mantocaja.create
int iCurrent
call super::create
end on

on w_mantocaja.destroy
call super::destroy
...
type gb_comandos from w_manto`gb_comandos within w_mantocaja
end type

type tab_todo from w_manto`tab_todo within w_mantocaja
end type

type tp_lista from w_manto`tp_lista within tab_todo
string text = "Lista de Cajas"
end type

type dw_lista from w_manto`dw_lista within tp_lista
string dataobject = "d_caja_lista"
end type
...
string text = "Datos de Caja"
end type

type dw_manto from w_manto`dw_manto within tp_manto
string dataobject = "d_caja_manto"
end type
...
choose case le_status	
	case new!,newmodified!
			If il_Fila > 0 then
				...
				messagebox("Advertecia " + Gn_Config.Is_Work.Aplicacion  ,"La Descripcion de la Caja  ya existe......!!!"+char(13)+"Fila : "+String(il_Fila)+"  C$$HEX1$$f300$$ENDHEX$$digo : "+ls_Codigo,Exclamation!)
...
type shl_cerrar from w_manto`shl_cerrar within w_mantocaja
end type

type shl_grabar from w_manto`shl_grabar within w_mantocaja
end type

type shl_eliminar from w_manto`shl_eliminar within w_mantocaja
end type

type shl_modificar from w_manto`shl_modificar within w_mantocaja
end type

type shl_nuevo from w_manto`shl_nuevo within w_mantocaja
end type

type uo_nav from w_manto`uo_nav within w_mantocaja
end type

type pb_cancelar from w_manto`pb_cancelar within w_mantocaja
end type

type pb_grabar from w_manto`pb_grabar within w_mantocaja
end type

type pb_eliminar from w_manto`pb_eliminar within w_mantocaja
end type

type pb_nuevo from w_manto`pb_nuevo within w_mantocaja
end type

type pb_cerrar from w_manto`pb_cerrar within w_mantocaja
end type

type shl_cancelar from w_manto`shl_cancelar within w_mantocaja
end type

type pb_modificar from w_manto`pb_modificar within w_mantocaja
end type
