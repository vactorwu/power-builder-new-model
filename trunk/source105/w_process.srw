HA$PBExportHeader$w_process.srw
forward
global type w_process from w_class
end type
type gb_comandos from groupbox within w_process
end type
type tab_todo from tab within w_process
end type
type tp_cabecera from userobject within tab_todo
end type
type dw_cabecera from u_dw within tp_cabecera
end type
type pb_borrar from picturebutton within tp_cabecera
end type
type pb_abrir from picturebutton within tp_cabecera
end type
type tp_cabecera from userobject within tab_todo
...
type tp_detalle from userobject within tab_todo
end type
type dw_detalle from u_dw within tp_detalle
end type
type tp_detalle from userobject within tab_todo
...
type tab_todo from tab within w_process
tp_cabecera tp_cabecera
tp_detalle tp_detalle
tp_lista tp_lista
end type
type shl_cerrar from statichyperlink within w_process
end type
type shl_grabar from statichyperlink within w_process
end type
type shl_eliminar from statichyperlink within w_process
end type
type shl_modificar from statichyperlink within w_process
end type
type shl_nuevo from statichyperlink within w_process
end type
type uo_nav from uo_navegacion within w_process
end type
type pb_cancelar from picturebutton within w_process
end type
type pb_grabar from picturebutton within w_process
end type
type pb_eliminar from picturebutton within w_process
end type
type pb_nuevo from picturebutton within w_process
end type
type pb_cerrar from picturebutton within w_process
end type
type shl_cancelar from statichyperlink within w_process
end type
type pb_modificar from picturebutton within w_process
end type
end forward
...
string menuname = "m_tppsoftmantoprocess"
boolean maxbox = true
...
Long act , deact
String is_columns[] ,  is_columnstag[],Is_Botones[]
...
//If tab_t.tp_manto.ole_1.activate(aat_Type) <> 0 Then
...
if this.MenuName = "m_tppsoftmantoprocess" then this.MenuID = create m_tppsoftmantoprocess
...
//inv_resize.of_Register(tab_todo.tp_cabecera.dw_manto, inv_resize.SCALERIGHTBOTTOM ) 
inv_resize.of_Register(tab_todo.tp_cabecera.pb_abrir, inv_resize.FIXEDRIGHT ) 
inv_resize.of_Register(tab_todo.tp_cabecera.pb_borrar, inv_resize.FIXEDRIGHT ) 
...
event ue_modify;call super::ue_modify;If ii_Modify = 1 Then
	idw_activa.setfocus( )
	Is_Botones[Li_Tab] =  'Modified'
	wf_botones (Is_Botones[Li_Tab])
...
event ue_postopen;call super::ue_postopen;if idw_lista.rowcount() > 0 then
	...
	tab_todo.tp_lista.em_buscar.setfocus( )
Else
	this.triggerevent('ue_newprocess')
End If
...
wf_centrardw(idw_detalle ,"X")
wf_centrardw(idw_lista,"X")

end event

event ue_newprocess;call super::ue_newprocess;If ii_New = 1 Then
	Is_Botones[Li_Tab] = 'New'
	wf_botones (Is_Botones[Li_Tab])
	idw_lista.enabled =  false
End If
end event

event ue_saveprocess;call super::ue_saveprocess;
If gb_AbrirCatalogos Then	
	If ii_Save = 1 Then
		If (idw_activa.enabled = false) then
			...
			if idw_activa.rowcount( ) > 0 then
				...
				End If
				This.idw_lista.ScrollToRow(this.idw_lista.find( Is_CampoCod+" = '"+Codigo+"'",1,this.idw_lista.rowcount()))
				idw_activa.setredraw(true)
				idw_lista.setredraw(true)
				Is_Botones[Li_Tab] =  'Save'
				wf_botones (Is_Botones[Li_Tab])
...
event ue_undoprocess;call super::ue_undoprocess;If ii_Undo = 1 Then
	if (idw_activa.enabled=  false) then
...
on tab_todo.create
this.tp_cabecera=create tp_cabecera
this.tp_detalle=create tp_detalle
this.tp_lista=create tp_lista
this.Control[]={this.tp_cabecera,&
this.tp_detalle,&
this.tp_lista}
end on

on tab_todo.destroy
destroy(this.tp_cabecera)
destroy(this.tp_detalle)
destroy(this.tp_lista)
end on

event selectionchanged;IF NewIndex = 1 OR  NewIndex = 3 THEN
	Li_Tab = 1
ELSE
	Li_Tab = NewIndex
END IF
...
string powertiptext = "Datos Principales de"
dw_cabecera dw_cabecera
pb_borrar pb_borrar
pb_abrir pb_abrir
end type

on tp_cabecera.create
this.dw_cabecera=create dw_cabecera
this.pb_borrar=create pb_borrar
this.pb_abrir=create pb_abrir
this.Control[]={this.dw_cabecera,&
this.pb_borrar,&
this.pb_abrir}
end on

on tp_cabecera.destroy
...
event constructor;call super::constructor;Is_Botones[1] = 'Save'
Is_Botones[2] = 'Save'
wf_dime_dw (this)
wf_centrardw(this,"X")
...
tab_todo.selecttab(tab_todo.tp_cabecera)
idw_activa.setfocus( )
...
on tp_detalle.create
this.dw_detalle=create dw_detalle
this.Control[]={this.dw_detalle}
end on

on tp_detalle.destroy
...
event constructor;call super::constructor;wf_dime_dwdetalle (this)
wf_centrardw(this,"X")
end event
...
event clicked;if shl_cancelar.enabled =  true then
	Parent.TriggerEvent('ue_undoprocess')
End If
end event
...
event clicked;if shl_grabar.enabled =  true then
	Parent.TriggerEvent('ue_saveprocess')
End If
end event
...
event clicked;if shl_eliminar.enabled =  true then
	Parent.TriggerEvent('ue_deleteprocess')
End If
end event
...
event clicked;if shl_nuevo.enabled =  true then
	Parent.TriggerEvent('ue_newprocess')
End If
end event
...
event clicked;if shl_modificar.enabled =  true then
	Parent.TriggerEvent('ue_modifyprocess')
End If
