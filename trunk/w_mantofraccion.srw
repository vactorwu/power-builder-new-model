HA$PBExportHeader$w_mantofraccion.srw
forward
global type w_mantofraccion from w_manto
end type
end forward

global type w_mantofraccion from w_manto
end type
global w_mantofraccion w_mantofraccion
...
//***************************************************************************************************************************************************************************************
SELECT Max(moneda.codmoneda)   INTO :ls_codigo  FROM moneda Using SQLCA  ;
...
on w_mantofraccion.create
int iCurrent
call super::create
end on

on w_mantofraccion.destroy
call super::destroy
...
type gb_comandos from w_manto`gb_comandos within w_mantofraccion
end type

type tab_todo from w_manto`tab_todo within w_mantofraccion
end type
...
choose case le_status	
	case new!,newmodified!
			If il_Fila > 0 then
				...
				messagebox("Advertecia " + Gn_Config.Is_Work.Aplicacion  ,"Descricpcion de Moneda ya existe......!!!"+char(13)+"Fila : "+String(il_Fila)+"  C$$HEX1$$f300$$ENDHEX$$digo : "+ls_Codigo,Exclamation!)
				Return 0
			Else
				Return 1
			End If
...
type shl_cerrar from w_manto`shl_cerrar within w_mantofraccion
end type

type shl_grabar from w_manto`shl_grabar within w_mantofraccion
end type

type shl_eliminar from w_manto`shl_eliminar within w_mantofraccion
end type

type shl_modificar from w_manto`shl_modificar within w_mantofraccion
end type

type shl_nuevo from w_manto`shl_nuevo within w_mantofraccion
end type

type uo_nav from w_manto`uo_nav within w_mantofraccion
end type

type pb_cancelar from w_manto`pb_cancelar within w_mantofraccion
end type

type pb_grabar from w_manto`pb_grabar within w_mantofraccion
end type

type pb_eliminar from w_manto`pb_eliminar within w_mantofraccion
end type

type pb_nuevo from w_manto`pb_nuevo within w_mantofraccion
end type

type pb_cerrar from w_manto`pb_cerrar within w_mantofraccion
end type

type shl_cancelar from w_manto`shl_cancelar within w_mantofraccion
end type

type pb_modificar from w_manto`pb_modificar within w_mantofraccion
end type
